...
namespace hicbit {
    ...
    */
     ...
     */
    //% weight=38 blockId=hicbit_lineSensorValue1 block="Get line follower sensor Value|port %port|value(0~255)"
    export function hicbit_lineSensorValue(port: hicbit_photosensitivePort): number {
        let ADCPin: AnalogPin;
        switch (port) {
            case hicbit_photosensitivePort.port1:
                ADCPin = AnalogPin.P1;
    ...
    */
    ...
    /**
    * Get the collision sensor status,1 trigger,0 no trigger
    */
    //% weight=16 blockId=hicbit_collisionsensor block="collision sensor|port %port|is trigger"    
    export function hicbit_collisionsensor(port: hicbit_PhotosensitiveSensor): boolean {
        let status = 0;
        let flag: boolean = false;
        switch (port) {
            case hicbit_PhotosensitiveSensor.port1:
    ...
    export function get_DHT11_value( port: hicbit_PhotosensitiveSensor,dhtResult: Dht11Result): number {
        ...
        switch (port) {
            ...
            case hicbit_PhotosensitiveSensor.port5:
                ...
                break;
            case hicbit_PhotosensitiveSensor.port7:
                dht11pin = DigitalPin.P6;
                break;
            case hicbit_PhotosensitiveSensor.port8:
                dht11pin = DigitalPin.P9;
                break;
        }
